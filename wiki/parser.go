...
func ParseToPackageName(pages []WikiPage, packageDir string) {
...
func ParseToOutDir(pages []WikiPage, outDir string) {
	...
	fmt.Println(pages, outDir)
	for _, d := range dirs {
		for _, page := range pages {
			files := []string{}
			outFile := path.Join(outDir, page.PageName+".markdown")
			fmt.Println(page.PageName, page.ApiFiles, page.FuncNames)
			for _, f := range page.ApiFiles {
				goFile := path.Join(d, "src", f)
				if _, err := os.Stat(goFile); err == nil {
					files = append(files, goFile)
